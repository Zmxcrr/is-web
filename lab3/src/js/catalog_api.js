...
function productToHTML(p) {
    ...
    <div class="catalog__item">
      <img src="${new URL(p.image, API_BASE)}" alt="${p.title}" loading="lazy" class="catalog__item-img">
      <div class="catalog__item-label">
        <div class="catalog__item-name">${p.title}</div>
        <div class="catalog__item-price">${p.price} €</div>
      </div>
    </div>`;
}
...
async function renderProducts(category = "tea") {
    const grid = document.querySelector(".catalog__content");
    if (!grid) return console.warn("No .catalog__content found");
